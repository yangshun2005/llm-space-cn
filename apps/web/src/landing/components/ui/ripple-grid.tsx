...
void main() {
    vec2 uv = vUv * 2.0 - 1.0;
    uv.x *= iResolution.y > 0.0 ? iResolution.x / iResolution.y : 1.0;
    ...
    const resize = () => {
      ...
      // Skip 0-sized layouts — a 0 height divides-by-zero in the shader
      // (iResolution.y == 0 -> NaN) and paints the whole quad white.
      if (w === 0 || h === 0) return;
    ...
    // The container can start at 0 height until layout settles; observe it so
    // iResolution is set from a real size before the first frames render.
    ...
    return () => {
      // Stop this mount's render loop so it can't keep drawing to a detached
      // context (StrictMode mounts the effect twice in dev).
      ...
      // Remove this effect's own canvas (no-op if already detached), so a
      // StrictMode re-mount never leaves a stale/white canvas behind.
      gl.canvas.remove();
    };
  }, []);
